...
def test_init(subscribe_mock: MagicMock, qtbot) -> None:
    ...
    assert data_file._record_btn.text() == "Start recording"
    assert data_file._record_btn.isEnabled()
...
def test_start_recording_path_dialog_cancelled(
    ...
    with patch.object(data_file._open_dir_widget, "try_get_path") as path_mock:
...
def test_on_file_open(data_file: DataFileControl, qtbot) -> None:
    ...
    data_file._on_file_open(FILE_PATH)
    assert data_file._record_btn.text() == "Stop recording"
